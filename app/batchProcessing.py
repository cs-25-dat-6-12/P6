...
def create_name_list(block, blocks_df, separator=";"):
    # given a set of record indexes, fetch each name from their dataframe and write a string where each name is separated by "separator"
    name_list = ""
    for index in block:
        name_list += f'{blocks_df.iloc[index]["title"]}' + separator
    name_list = name_list[: -len(separator)]
    return name_list
...
def name_parts_to_string(name_parts, separator=";"):
    # given a dictionary of name parts, convert it into a string
    string = ""
    for part in name_parts:
        string += part + ": "
        string += name_parts[part] + separator
    string = string[: -len(separator)]
    return string


def column_name_parts_to_string(
    row, column_names=["placeholder", "other placeholder"], separator=";"
):
    ...
    for part_name in column_names:
        ...
        if name_part != None and name_part == name_part:
            string += (
                part_name.removeprefix("name_part_").removesuffix("_phoneme") + ": "
            )
            string += name_part + separator
    string = string[: -len(separator)]
    return string
...
def prepare_batch_file_filter_lists(
    blocks,
    df,
    blocks_df,
    filepath,
    ...
    max_list_size=10,
    leftover_blocks_filepath=r"app\leftoverBlocks.jsonl",
):
    # given a blocks-dictionary as generated by textBlocking.py, the dataset the blocks were made for "df",
    # and the dataset used to create those blocks "blocks_df", write a file with gpt-4.1-nano requests for batch processing.
    # Will split blocks into lists of at most max_list_size names and not write requests for lists with only one element.
    # Writes a file with blocks consisting of the records that did not get included in any lists
    print(f"Overwriting {filepath}")
    ...
    with open(filepath, "w", encoding="utf-8") as file:
        # make a copy of the blocks to avoid modifying the original blocks later
        new_blocks = blocks.copy()
        for record in blocks:
            # create sub-blocks to extract names from
            block = blocks[record]
            sub_blocks = []
            i = 0
            while True:
                start = i * max_list_size
                end = start + max_list_size
                sub_block = block[start:end]
                i += 1
                if not len(sub_block) > 1:
                    new_blocks.update({record: sub_block})
                    break
                sub_blocks.append(sub_block)
            for i, sub_block in enumerate(sub_blocks):
                print(
                    f"Writing request for block {record}-{i}      ",
                    end="\r",
                )
                request = {
                    "custom_id": f"{record}-{i}",
                    "method": "POST",
                    "url": "/v1/chat/completions",
                    "body": {
                        "model": model,
                        "messages": [
                            ...
                            {
                                "role": "user",
                                "content": f"The target name is {df.iloc[record]["title"]}.",
                            },
                            {
                                "role": "user",
                                "content": "Candidates: "
        ...
        print(f"Writing leftover comparisons to {leftover_blocks_filepath}...")
        with open(leftover_blocks_filepath, "w", encoding="utf-8") as file:
            json.dump(new_blocks, file, ensure_ascii=False, indent=4)
        print("All done!")


def prepare_batch_file_individual_pairs(
    blocks,
    df,
    blocks_df,
    filepath,
        ...
        for record in blocks:
            for possible_match in blocks[record]:
                try:
                    ...
                    request = {
                        "custom_id": f"{record}#{possible_match}",
                        "method": "POST",
                        "url": "/v1/chat/completions",
                        "body": {
                            "model": model,
...
if __name__ == "__main__":
    df = pd.read_csv(r"datasets\phonetic\LASKI_phonetic.csv", sep=",", header=0)
            ...
            case "write":
                ...
                match sub_choice:
                    ...
                    case "lists":
                        ...
                        prepare_batch_file_filter_lists(
                            blocks,
                            df,
                            blocks_df,
                            "app/" + filename + ".jsonl",
