...
def send_data_to_service(
    df: pd.DataFrame, config: Dict, service_name: str
    ...
    print(
        "Sending {} MB to {}".format(sys.getsizeof(data_json) / 1000000, service_name)
    )
    max_retries = 3
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.response.status_code == 413:  # Check if error is due to large request
                ...
                result_first_half = send_data_to_service(
                    first_half, config, service_name
                )
                result_second_half = send_data_to_service(
                    second_half, config, service_name
                )
                ...
                print(
                    f"Error occurred after {max_retries} attempts: {e}. Sending a warning and continuing..."
                )
                return None
...
def prep_df(df: pd.DataFrame):
    ...
    config = {
        "trans_flag": False,
        "phonetic_flag": True,
        "extended_rules_flag": True,
        "remove_vowels_flag": False,
    }
    ...
    if df["id"].isnull().sum() > 0:
        print(
            f"Warning: {df['id'].isnull().sum()} NaN id values found in the dataframe. These will be removed."
        )
    ...
    for field in prep_fields:
        if field not in df.columns:
            raise ValueError(
                f"Dataframe must contain the following fields: {prep_fields} and contains {df.columns}"
            )
    ...
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(send_data_to_service, df, config, service) for df in df_list
        ]
...
def translit(data, name, lang):
    titles = list(data["title"])
    tr_titles = []
    for title in titles:
        results = tr([title], lang)
        tr_titles.append(results[0])
    data["title"] = pd.Series(tr_titles)

    name_parts_dicts = list(data["name_parts"])
    tr_name_parts_dicts = []
    for name_parts in name_parts_dicts:
        try:
            tr_name_parts = {}
